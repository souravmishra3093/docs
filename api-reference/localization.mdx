...
description: "Localization is the REST API developed for localizing the contents from English to Indic languages. Using the Localization API, you can either localize a single sentence or multiple sentences in a fraction of time."
...
<ParamField header="domain" type="string" required>
  {" "}
  - The Domain refers to the universe in which you use the Localization API. Example- Health Care, Insurance, Legal, etc.
...
<ParamField header="apiStatus" type="integer">
  {" "}
  - Status of API Moderated / Partial /Unmoderated (Aggregate of all segments). Refer to [API Messages](/usage-guides/localization/api-messages) to know about the available apiStatus and its description.
</ParamField> 
...
<ParamField header="api_statuses" type="Map<String, Integer>">
  ...
  - Refer to section [Language Codes](/usage-guides/localization#supported-languages) for valid language code.
  - Refer to [API Messages](/usage-guides/localization/api-messages) to know about the available apiStatus and its description.
...
Refer to: [API Messages](/usage-guides/localization/api-messages) section to know the error code and corresponding messages.
