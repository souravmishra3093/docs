...
The feature to screen the non-dictionary words used in a sentence. In other words, the mask will indicate the words that should not be translated into the target language. Note - By default, the nmtMask = false. Note - To set the nmtMask = true, it is mandatory the src_lang = en (English).
